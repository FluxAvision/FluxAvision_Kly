...
- **硬件指纹绑定** — 数据库密钥绑定 CPU/主板/磁盘/MAC，拷贝到其他机器无法使用
- **RTSP 视频流预览** — OpenCV 拉流转 MJPEG，浏览器直接播放
...
├── next.config.ts                   # Next.js 配置（开发模式 rewrites 代理到 8080）
...
│                                                          │
│  第2层: 数据库密钥保护                                     │
│  └─ AES-256 密钥以硬件指纹派生密钥加密，存储在 .key 文件   │
