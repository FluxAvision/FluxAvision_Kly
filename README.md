...
最终交付为 **Windows exe 安装�?*，支持后台静默运行、开机自启、系统托盘管理�?
...
- **Ed25519 数字签名授权** �?外部平台根据硬件指纹生成激活码，不可伪造、不可破�?
- **AES-256 字段加密** �?设备密码、登录密码、激活码等敏感字段透明加解�?
- **硬件指纹绑定** �?数据库密钥绑�?CPU/主板/磁盘/MAC，拷贝到其他机器无法使用
- **RTSP 视频流预�?* �?OpenCV 拉流�?MJPEG，浏览器直接播放

### 管理功能
- 设备管理（增删改查、SDK 连接状态实时监控）
- 系统设置（门店名称、登录密码、仪表盘指标配置�?
- 客流数据查询（按小时/�?设备/日期范围筛选）
- 大屏展示（自定义标题、指标、设备筛选）
- 一键生成测试数据（开发调试用�?

### 打包部署
- **PyInstaller** 打包�?Windows exe（onedir 模式�?
...
�?  ├── config.py                    # 全局配置（路�?密钥/硬件指纹�?
�?  ├── database.py                  # SQLite3 数据库连�?+ 迁移
�?  ├── field_crypto.py              # AES-256 字段加解密工�?
...
�?  └── logo.svg                     # SVG �?Logo
�?
├── package.json                     # Node.js 依赖和脚�?
├── next.config.ts                   # Next.js 配置（开发模�?rewrites 代理�?8080�?
...
| GET | `/api/traffic/dashboard` | 仪表盘聚合数�?|
| GET | `/api/traffic-collector/status` | 采集器状态详�?|
| POST | `/api/traffic-collector/restart` | 重启采集�?|
| GET | `/api/settings` | 获取系统设置 |
| PUT | `/api/settings` | 更新系统设置 |
| GET | `/api/license` | 获取 License 信息（含硬件指纹�?|
| POST | `/api/license` | 激�?License（Ed25519 验证�?|
| GET | `/api/license/parse` | 解析激活码（调试用�?|
| POST | `/api/seed` | 生成测试数据 |
| GET | `/api/devices/:id/stream` | RTSP 视频�?(MJPEG) |
...
浏览器打开 `http://localhost:3000`，前端通过 rewrites 自动代理 `/api/*` 到后�?8080�?

### 5. 大华 SDK（可选）

如果有大�?NetSDK wheel 包，放入 `backend/` 目录后安装：

```bash
pip install backend/NetSDK-2.0.0.1-py3-none-win_amd64.whl
```

> SDK �?Windows x64 可用，Linux/macOS 会自动跳过�?
...
`build.bat` 会自动执行以�?6 步：

| 步骤 | 说明 |
|------|------|
| 1/6 | 检�?Python + Node.js 环境 |
| 2/6 | 安装 Python 打包依赖（PyInstaller、pystray、Pillow�?|
| 3/6 | 安装 Node.js 前端依赖 |
| 4/6 | 构建 Next.js 前端（临时切换为 export 模式，构建完恢复�?|
| 5/6 | 生成应用图标 |
| 6/6 | PyInstaller 打包�?exe |

构建产物�?`backend/dist/FluxAvision/FluxAvision.exe`�?
...
# 2. 构建前端（需要临时修�?next.config.ts �?output �?"export"�?
cd ..
npx next build
...
载荷包含：硬件指纹前8�?+ 最大路�?+ 授权天数 + 签发时间戳�?

---

## 数据目录说明

运行时自动在项目根目录（�?exe 同级目录）创�?`data/` 目录�?

```
data/
├── FluxAvision.db          # SQLite3 数据库文�?
...
> 数据库中敏感字段（设备密码、登录密码、激活码）存储为 `ENC:` 前缀�?AES-256 密文�?
...
�?                                                         �?
�? �?�? 硬件指纹绑定                                      �?
�? └─ CPU序列�?+ 主板序列�?+ 磁盘序列�?+ MAC �?SHA-256  �?
�?                                                         �?
�? �?�? 数据库密钥保�?                                    �?
�? └─ AES-256 密钥以硬件指纹派生密钥加密，存储�?.key 文件   �?
�? └─ 硬件变更 �?密钥验证失败 �?自动重建（防拷贝�?         �?
�?                                                         �?
�? �?�? 敏感字段加密                                       �?
�? └─ password / loginPassword / activationCode            �?
�? └─ AES-256-CBC + PKCS7，每次加密使用随�?IV              �?
�?                                                         �?
�? �?�? Ed25519 数字签名授权                               �?
�? └─ 公钥内置客户端，私钥仅授权平台持�?                     �?
�? └─ 激活码包含指纹匹配 + 路数/天数限制                     �?
...
| 问题 | 解决方案 |
|------|----------|
| `pip install` 报错 | 确认 Python 版本 3.8+，尝�?`pip install --upgrade pip` |
| 端口 8080 被占�?| 修改 `backend/config.py` �?`API_PORT` 或用 `--port` 参数 |
| 前端代理 502 | 确认后端已启动在 8080 端口 |
| 大华 SDK 不可�?| 仅支�?Windows x64，确认已安装 `.whl` �?|
| 数据库解密失�?| 删除 `data/.key` �?`data/FluxAvision.db*` 重新初始�?|
| PyInstaller 打包失败 | 确认 `frontend-build/` 目录存在且有 `index.html` |
| 打包后前端空�?| 检�?`FluxAvision.spec` �?`datas` 路径是否正确 |
